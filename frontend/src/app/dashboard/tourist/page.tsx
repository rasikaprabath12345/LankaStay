        ...
        <div className="space-y-6">
          {bookings.map((booking) => (
            ...
            >
              ...
              {/* Action Buttons */}
              <div className="flex flex-col sm:flex-row md:flex-col justify-center items-stretch md:items-end shrink-0 gap-2.5">
                {/* WhatsApp Follow-up for Active, Pending or Confirmed bookings */}
                {(booking.status === 1 || booking.status === 2 || booking.status === 3) && (
                  <a
                    href={`https://wa.me/94771234567?text=Hi!%20I%20have%20a%20booking%20(Ref:%20${booking.id})%20for%20"${encodeURIComponent(booking.experienceTitle)}"%20on%20LankaStay%20from%20${new Date(booking.startDate).toLocaleDateString()}%20to%20${new Date(booking.endDate).toLocaleDateString()}.%20Let's%20discuss%20the%20stay!`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="rounded-xl bg-[#25D366] hover:bg-[#20ba5a] text-white px-4 py-2.5 text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 shadow-sm active:scale-95"
                  >
                ...
                {(booking.status === 1 || booking.status === 2) && (
                  <button
                    ...
                    className="rounded-xl border border-rose-250 hover:bg-rose-50 px-4 py-2.5 text-xs font-bold text-rose-600 transition-all text-center"
                  >
                    Cancel Booking
                  </button>
                )}
